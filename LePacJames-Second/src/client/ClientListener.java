...
public class ClientListener extends Thread {
    ...
    private void doConnect() {
        try {
            socket = new Socket(InetAddress.getByName(address), PORT.toInt());
    ...
    private void sendChat() {
        if (game.hud.tfMessage == null)
            return;
        
        String msg = game.hud.tfMessage.getText();
        try {
            if (msg.length() < 1) {
                oos.writeObject("CHAT:EMPTY");
                oos.flush();
            } else {
                oos.writeObject("CHAT:" + msg);
                oos.flush();
                game.hud.tfMessage.clear();
            }
    ...
    private void receiveChat() {
        ...
        try {
            ...
            if (!message.equals("EMPTY")) {
                Platform.runLater(() -> {
                    game.hud.taChat.appendText(message + "\n");
                });
